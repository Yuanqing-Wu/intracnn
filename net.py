...
class Net32x32(torch.nn.Module):
    def __init__(self):
        ...
        self.NC3 = nn.Conv2d(64, 128, kernel_size=4, stride=4, padding=0, bias=True) # (64, 2, 2) -> (128, 1, 1)

        self.FC1_stage1 = nn.Linear(129,32) # 129 -> 32
        self.FC1_stage2 = nn.Linear(129,32) # 129 -> 32

        self.FC2_stage1 = nn.Linear(33,2) # 33 -> 2
        self.FC2_stage2 = nn.Linear(33,2) # 33 -> 2
    ...
    def forward(self, org, pre, qp):
        ...
        x1 = F.relu(self.FC2_stage1(torch.cat((x1, qp), dim=1)))

        x2 = F.relu(self.FC1_stage2(torch.cat((T, qp), dim=1)))
        x2 = F.relu(self.FC2_stage2(torch.cat((x2, qp), dim=1)))
...
class Net32x16(torch.nn.Module):
    def __init__(self):
        ...
        self.NC_o1 = nn.Conv2d(1, 8, kernel_size=(8, 4), stride=4, padding=0, bias=True) # (1, 32, 16) -> (8, 4, 4)
        self.NC_p1 = nn.Conv2d(2, 8, kernel_size=(8, 4), stride=4, padding=0, bias=True) # (2, 32, 16) -> (8, 4, 4)
...
class Net8x8(torch.nn.Module):
    def __init__(self):
        ...
        self.NC2 = nn.Conv2d(16, 64, kernel_size=2, stride=2, padding=0, bias=True) # (16, 2, 2) -> (32, 1, 1)
...
class Net32x8(torch.nn.Module):
    def __init__(self):
        ...
        self.NC_o1 = nn.Conv2d(1, 8, kernel_size=(16, 4), stride=4, padding=0, bias=True) # (1, 32, 8) -> (8, 2, 2)
        self.NC_p1 = nn.Conv2d(2, 8, kernel_size=(16, 4), stride=4, padding=0, bias=True) # (2, 32, 8) -> (8, 2, 2)
...
class Net16x8(torch.nn.Module):
    def __init__(self):
        ...
        self.NC_o1 = nn.Conv2d(1, 8, kernel_size=(8, 4), stride=4, padding=0, bias=True) # (1, 16, 8) -> (8, 2, 2)
        self.NC_p1 = nn.Conv2d(2, 8, kernel_size=(8, 4), stride=4, padding=0, bias=True) # (2, 16, 8) -> (8, 2, 2)
...
class Net32x4(torch.nn.Module):
    def __init__(self):
        ...
        self.NC_o1 = nn.Conv2d(1, 8, kernel_size=(16, 2), stride=4, padding=0, bias=True) # (1, 32, 4) -> (8, 2, 2)
        self.NC_p1 = nn.Conv2d(2, 8, kernel_size=(16, 2), stride=4, padding=0, bias=True) # (2, 32, 4) -> (8, 2, 2)
...
class Net16x4(torch.nn.Module):
    def __init__(self):
        ...
        self.NC_o1 = nn.Conv2d(1, 8, kernel_size=(8, 2), stride=4, padding=0, bias=True) # (1, 16, 4) -> (8, 2, 2)
        self.NC_p1 = nn.Conv2d(2, 8, kernel_size=(8, 2), stride=4, padding=0, bias=True) # (2, 16, 4) -> (8, 2, 2)
...
class Net8x4(torch.nn.Module):
    def __init__(self):
        ...
        self.NC_o1 = nn.Conv2d(1, 8, kernel_size=(4, 2), stride=4, padding=0, bias=True) # (1, 8, 4) -> (8, 2, 2)
        self.NC_p1 = nn.Conv2d(2, 8, kernel_size=(4, 2), stride=4, padding=0, bias=True) # (2, 8, 4) -> (8, 2, 2)
